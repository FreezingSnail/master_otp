defmodule Mastery.Boundary.Proctor do
  ...
  def handle_call({:schedule_quiz, quiz}, _from, quizzes) do
    ...
    build_reply_with_timeout({:reply, :ok}, ordered_quizes, now)
  end

  defp build_reply_with_timeout(reply, quizzes, timeout) do
    reply
    |> append_state(quizzes)
    |> maybe_append_timeout(quizzes, timeout)
  end
  ...
  def handle_info(:timeout, quizzes) do
    now = DateTime.utc_now()
    remainging_quizzes = start_quizzes(quizzes, now)
    build_reply_with_timeout({:noreply}, remainging_quizzes, now)
  end
