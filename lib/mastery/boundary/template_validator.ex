defmodule Mastery.Boundary.TemplateValidator do
  ...
  def validate_raw(raw) when is_binary(raw), do: :ok
  def validate_raw(_raw), do: {:error, "must be a binary"}
  ...
  def validate_generator(_generator), do: {:error, "must be an string to list of function pair"}

  def validate_checker(checker) when is_function(checker, 2), do: :ok
  def validate_checker(_checker), do: {:eoor, "must be a arity 2 function"}
end
