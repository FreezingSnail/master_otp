defmodule QuizTest do
  ...
  describe "when a quiz has two templates" do
    ...
    test "the next question is randomly selected", %{quiz: quiz} do
      ...
      assert other_template != first_template
    end

    test "templates are unique until cycle repeates", %{quiz: quiz} do
  ...
  describe "a quiz that always ads one and two" do
  ...
  def right_answer(quiz), do: answer_question(quiz, "3")
  def wrong_answer(quiz), do: answer_question(quiz, "wrong")
